...
- Installer/package: not yet built for this version — build after the user confirms the interactive behavior (arrow-key scrubbing, sensitivity feel, cube usability) in the running app, per this repo's usual PG 매칭-mode-style rollout.
- Signature status: still unsigned.
- Out of scope (left untouched): actual code-signing; lathe (2-axis) coordinate mapping; G90/G91 incremental-mode support for ordinary moves; a settings UI for cube face labels/colors.
