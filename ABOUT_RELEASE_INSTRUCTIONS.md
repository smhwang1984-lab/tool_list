...
- Summary: 사용자 요청 3건(2026-09-06), 실제 프로그램 `O3230.nc`로 가공시간
  버그 원인 확정.
  1. **가공시간 — 선반 G98/G99가 모션 코드를 덮던 버그(핵심)**. "선반에서
     극좌표나 밀링 가공 시 mm/min속도 계산이 맞지 않는 것으로 보임 지금
     테스트 했던 파일에서 13공정 시간이 2시간이 넘음". 밀링/MCT의
     `G98/G99`는 고정 사이클 복귀 레벨이라 만나면 `current_motion`을
     `"G98"`로 덮어써도 맞지만, **선반의 G98/G99는 이송 단위**(mm/min↔
     mm/rev)일 뿐 모션 코드가 아니다. 이 처리를 선반에도 그대로 태우면
     자기 G-워드 없는 모달 이동(보통 위치 복귀 G00)이 절삭 이동으로
     오인되고, 직전 나사가공의 mm/rev 값(F1.5875)이 mm/min으로 잘못
     적용됐다(`O3230.nc:486` "G98X100.Z10.T0404"). 선반은 이 블록을
     건너뛰게 고쳤다 — `g98_active` 자체는 선반에서 아무도 읽지 않는다.
     방어선으로, 이송 단위가 실제로 바뀐 줄에 새 F가 없으면 이전 F를
     무효화한다(mm/rev↔mm/min 숫자를 그대로 넘기면 시간이 어긋나므로).
  2. **선반 SO 열**. "추후 선반 주석에 인서트 부분에 [SO nn] 값을
     넣겠음 넣은 so 길이를 홀더와 remark 사이에 SO를 넣어서 그 데이터를
     넣어줘". `LATHE_COLUMNS`가 `NO/INSERT/HOLDER/SO/REMARK` 5열이 됐다.
     홀더/인서트 두 주석 줄 중 어느 쪽에 `[SO nn]`이 있어도 읽어 SO 열에
     넣고, 표시 문구에서는 그 표기를 걷어낸다.
  3. **산출 모드 콤보 폭**. "툴리스트 산출 모드에서 선반/밀링 드롭선택에서
     보이는 창이 문자가 가려져 드롭창의 가로길이를 늘려 문자 가려짐 없게
     설정". `tool_mode_combo`에 항목 실측 폭 + 여유를 최소 폭으로 걸고
     `AdjustToContents`를 켜, 다크/라이트 공용 QSS 테두리 때문에 화살표가
     글자를 가리던 문제를 없앴다.
- Creator displayed: Hwang.seonmun
- Open source used: Python, PyQt5, pyqtgraph, NumPy, PyOpenGL, ReportLab, PyInstaller,
  Inno Setup (새 의존성 없음).
- Details:
  - **`nc_viewer_widget.py`**: `if g98_pattern.search(...): current_motion =
    "G98"` 블록을 `if not is_lathe:`로 감쌌다(2946행 부근). 선반 전용
    이송 단위 판정 블록(2875행 부근)에 `prev_feed_per_rev` 비교를 추가해,
    `lathe_feed_per_rev`가 이 줄에서 실제로 바뀌었고(`f_match`가 없으면)
    `current_feed = 0.0`으로 되돌린다.
  - **`NC_Tool_List.py`**: `LATHE_SO_RE`(밀링 `TOOL_RE`와 같은 `[SO nn]`
    표기)를 추가하고 `parse_lathe_program()`에서 홀더/인서트 주석 두 줄
    모두를 검사해 먼저 나오는 SO 값을 쓰고 표기를 걷어낸다. `LATHE_COLUMNS`/
    `_LATHE_COL_WIDTH_BASE`/`LATHE_PDF_COLUMN_WEIGHTS`에 SO를 추가하고,
    `make_lathe_pdf_info_row()`의 4칸 하드코딩을 `len(LATHE_COLUMNS)`
    기반으로 고쳐 열 개수가 바뀌어도 안전하게 했다. `style_lathe_pdf_table()`
    에서 SO 열만 TOOL NO처럼 가운데 정렬한다. `tool_mode_combo`에
    `TOOL_MODE_COMBO_EXTRA_PX`(46px) 상수 기반 최소 폭과
    `AdjustToContents`를 걸었다.
- Tests: `tests/test_nc_tool_list.py` 191개 통과(v1.7.1의 180 + 신규 11,
  무관한 `SingleInstanceTests` 1건은 여전히 이 PC의 실행 중인 인스턴스
  때문에 환경 요인으로 실패). 신규는 `LatheModeTests`에 추가: G98 바닥줄이
  모션 타입을 오염시키지 않음(핵심 회귀 테스트), 이송 단위 전환 시 F
  무효화(양방향 — 같은 줄 F 있으면 유지), MCT의 G98은 종전대로 사이클
  복귀로 동작(밀링 회귀 방지), `O3230.nc` N12~N13 축약 실사례(시간이
  수십 초 이내), SO 파싱 3케이스(인서트 줄/홀더 줄/부재), 열 스키마,
  PDF 가중치·정보행 칸수 일치; `App` 테스트에 콤보 최소 폭 확인 1건 추가.
  기존 표 스키마 테스트(4열→5열)도 갱신.
- Installer/package status: **생성 완료** (사용자 직접 지시로 즉시 빌드).
  - `python -m PyInstaller --noconfirm --clean NC_Tool_List.spec` — onedir, UPX 비활성,
    `dist\NC_Tool_List` 151 MB, `_internal\OpenGL` 폴더 부재 유지(freeglut/gle32/64
    DLL 배제, MSVCR90.dll 경고는 기존과 동일하게 무해).
  - `ISCC.exe NC_Tool_List.iss` (Inno Setup 6) — `installer\NC_Tool_List_Setup_v1.7.2.exe`
    46.5 MB.
  - 포터블: `installer\NC_Tool_List_Portable_v1.7.2.zip` 64.0 MB, 315개 항목
    (이전 버전과 같은 구성 — 새 의존성 없음).
  - 빌드 검증: 프리즈된 `NC_Tool_List.exe`의 파일/제품 버전이 `1.7.2.0`으로 찍히고,
    실행하면 `startup.log`에 트레이스백 없이 `Starting Sum Path v1.7.2 frozen=True`가
    남는 것을 확인했다(이 PC에 이미 떠 있던 인스턴스로 정상 핸드오프 후 종료).
    설치 프로그램 자체의 VersionInfo도 `1.7.2`로 확인.
  - **실측 검증(O3230.nc)**: 13번 공정(`P013_T04`)이 2시간 넘던 것에서
    118.01초로, 전체 14공정 합계가 807.9초(약 13.5분)로 정상화된 것을 확인.
  - Installer SHA-256: 496340C158FFB98FFDB5C145A531EEBBD0AD490797A61A3FC08247D88792A91F
  - Portable ZIP SHA-256: 803B00B3011CF20C929CF416E73D86F6C0A1C9688EA9B019A55480FC9203A967
  - App SHA-256: 42BBB53E4E42BBDBD0782A787E58FA8379CD1ADAE1B6ABAFC6A6D0E526E0019B
  - Signature status: still unsigned.
